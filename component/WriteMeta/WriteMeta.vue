<template>
  <form @submit.prevent="storeArticle">
    <ui-accordion title="General" v-model:open="accordion.general">
      <div class="grid md:grid-6-6">
        ...
        <div>
          <ui-input required label="Category" option-value="id" option-title="name" :options="categories"
                    placeholder="Choose category" type="select" v-model:value="article.category_id"></ui-input>
        </div>
        <div class="place-y-center md:m-l-5">
          <div class="text-accent m-t-1">Keywords</div>
          <div class="d-inline-block">
            <ui-tag v-if="article.keywords&&article.keywords.length>0" @removed="removeKeyword(keyword)" class="m-t-1"
                    color="accent-filled" removable v-for="keyword in article.keywords.split(',')">{{ keyword }}
            </ui-tag>
            <div class="b-1 p-x-2 d-inline-block bg-white m-t-1" style="border-radius: 15px; ">
              <input @blur="keywordProcess($event,true)" @keyup="keywordProcess" v-model="newKeyword"
                     placeholder="add keyword (comma-separated)" type="text" class="b-0">
            </div>
        ...
        <div class="d-flex">
          <ui-input class="f-1 place-y-center"
                    :label="'Public on blua.blue ' + (!article.publish_date?'once published':'')" type="checkbox"
                    v-model:value="article.is_public"></ui-input>
          <ui-icon @click="infoModal=true" title="click"
                   class="font-md cursor-pointer text-primary m-l-2 place-y-center p-b-2">info
          </ui-icon>
        ...
        <div class=" md:m-l-5">
          ...
          <div><span class="font-strong text-primary" v-if="article.slug">{{ article.slug }}</span><i class="font-light"
                                                                                                      v-else>..will be
            generated on save..</i></div>
        </div>
      </div>
    </ui-accordion>
    <ui-accordion v-model:open="accordion.image" title="Post image">
    ...
    <ui-accordion v-model:open="accordion.webhooks" title="Webhooks">
      <p class="font-sm">Webhooks can be created in your
        <router-link :to="'/profile/'+user?.user_name+'#webhooks'">profile</router-link>
      </p>
    ...
    <teleport to="body">
      <ui-modal :show="infoModal" title="Pubic vs Published" @close="infoModal=false">
        <div class="p-3">
          ...
          <p>Each article can be saved as a draft prior to publishing it. Other than your login (including API, of
            course), access to your article isn't possible.</p>
          <p>As soon as an article is published, blua.blue considers it as "live". However, this doesn't mean people can
            find or access it via blua.blue:</p>
          <h3>Public</h3>
          <p>
            The public flag distinguishes between published content that is used somewhere else (like via API or
            webhook) only and content that is available via this blua.blue webdomain.
            <br>Without being <strong>published</strong>, content cannot be seen by others regardless of public-state.
            BUT: publicity impacts your (SEO-)strategy.
            On one hand, being public within the realms of blua.blue can provide exposure and readers.
            On the other hand, you may want your content to be canonical (interpreted as original) on your own
            web-presence.
          </p>
          <h3>Versioning</h3>
          <p>
            As of now, versioning is not included in blua.blue. If you make changes to a published article, they will be
            live. If you set a previously published article as a draft,
...
export default {
  ...
  setup({article}) {
    ...
    const filledOutGeneral = () => article.name.length > 2 && article.teaser.length>2 && article.category_id
    ...
    const accordion = Vue.ref({general: !filledOutGeneral(), image: !article.image_id, webhooks:false})
    ...
    return {categories, newKeyword, webhooks, infoModal, deleteModal, saveNotification, user, accordion}
  },
  methods: {
    toggleNotification() {
