<template>
  <div class="container">
    <h1>Medium plugin</h1>
    <p>This plugin works as a proxy to facilitate blua.blue <-> medium.com communication.</p>
    ...
    <div v-if="process.step===0">
      ...
      <div class="grid-7-5">
        <form @submit.prevent="testLinking">
          <div class="m-y-2">
            <ui-input required label="Medium token" v-model:value="form.apiKey"/>
          </div>
        ...
        <div class="m-l-3">
          <p>You can find or generate your api key for medium at https://medium.com/me/settings under "Integration
            tokens"</p>
        </div>
      </div>
    </div>
    <div v-if="process.step>0">
      ...
      <div class="grid-7-5">
        <section>
          <div class="m-y-2">
            <ui-input required label="Reference name (can be anything you want)" v-model:value="process.name"/>
          </div>
          <div class="m-y-2">
            <ui-input required label="Endpoint" value="Plugin::Medium"/>
          </div>
          <div class="m-y-2">
...
export default {
  ...
  setup() {
    ...
    store.subscribe('auth', auth => {
      if (auth.length > 0) {
        profile = 'profile/' + auth[0].user.user_name + '#webhooks'
        profileLink.value = '{{base}}' + profile
        process.value.step = 0;
      }
    })

    const testLinking = () => {
      API.get(`/medium/${form.value.apiKey}`).then(res => {
